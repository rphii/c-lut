...
#define LUT_INCLUDE(N, A, TK, MK, TV, MV) \
    ...
    void A##_free(N *lut); \
    void A##_clear(N *lut); \
    ErrDecl A##_grow(N *lut, size_t width); \
    N##Item *A##_once(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val); \
    ErrDecl A##_set(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val); \
    ...
    ErrDecl A##_dump(N *lut, N##Item ***items, size_t *len); \
...
#define LUT_IMPLEMENT_COMMON_ITER_ALL(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    inline N##Item **A##_iter_all(N *lut, N##Item **item) { \
        LUT_ASSERT_ARG(lut); \
...
#define LUT_IMPLEMENT_COMMON_STATIC_GET_ITEM(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    inline static N##Item **A##_static_get_item(N *lut, const LUT_ITEM(TK, MK) key, size_t hash, bool intend_to_set) { \
        LUT_ASSERT_ARG(lut); \
...
#define LUT_IMPLEMENT_COMMON_DUMP(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    ErrImpl A##_dump(N *lut, N##Item ***items, size_t *len) { \
        LUT_ASSERT_ARG(lut); \
...
#define LUT_IMPLEMENT_COMMON_GROW(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    ErrImpl A##_grow(N *lut, size_t width) { \
        LUT_ASSERT_ARG(lut); \
...
#define LUT_IMPLEMENT_COMMON_ONCE(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    inline N##Item *A##_once(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val) { \
        LUT_ASSERT_ARG(lut); \
        LUT_ASSERT_ARG_M(key, MK); \
...
#define LUT_IMPLEMENT_COMMON_SET(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    ErrImpl A##_set(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val) { \
        LUT_ASSERT_ARG(lut); \
        LUT_ASSERT_ARG_M(key, MK); \
        if(2 * lut->used >= LUT_CAP(lut->width)) { \
...
#define LUT_IMPLEMENT_COMMON_GET(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    inline TV *A##_get(N *lut, const LUT_ITEM(TK, MK) key) { \
        LUT_ASSERT_ARG(lut); \
        LUT_ASSERT_ARG_M(key, MK); \
        if(!lut->width) return 0; \
        size_t hash = H(key); \
...
#define LUT_IMPLEMENT_COMMON_GET_KV(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    inline N##Item *A##_get_kv(N *lut, const LUT_ITEM(TK, MK) key) { \
        LUT_ASSERT_ARG(lut); \
        LUT_ASSERT_ARG_M(key, MK); \
        if(!lut->width) return 0; \
        size_t hash = H(key); \
...
#define LUT_IMPLEMENT_COMMON_DEL(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    inline void A##_del(N *lut, const LUT_ITEM(TK, MK) key) { \
        LUT_ASSERT_ARG(lut); \
        LUT_ASSERT_ARG_M(key, MK); \
        if(!lut->width) return; \
        size_t hash = H(key); \
